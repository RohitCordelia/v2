...
// @ts-ignore
import { useGetItineraryMutation, useCabinPricingMutation, useCreateBookingMutation, useGetUpgradeMutation, useGetItineraryByIdMutation } from '/src/services/weekend/weekend';
// @ts-ignore
...
export default function WeekendItinerary({ }: Props) {
  ...
  let itineraryId = location?.state?.itinerary
  ...
  useEffect(() => {
    ...
    let url = '';
    if (itineraryId) {
      const query = itineraryId.map(id => `itinerary_id[]=${encodeURIComponent(id)}`).join("&");
      url = `${query}`
    } else {
      url = `${night ? 'nights=' + night + '&' : ''}pagination=false${route_id ? '&route_id=' + route_id : ''}${round_trip ? '&round_trip=' + round_trip : ''}`
    }
  ...
  const proceedToBilling = async () => {
    if (isRoomSelected(selectedRooms)) {
      ...
      const updatedData = callUpgrade?.rooms?.map(item => {
        // find free upgrade (cabin_fare = 0)
        const freeUpgrade = item.upgrades.find(up => up.cabin_fare === 0);
        ...
        if (freeUpgrade) {
          return {
            ...
            category_details: {
              ...freeUpgrade,
              // keep the shape consistent with category_details
              code: freeUpgrade.code,
              id: freeUpgrade.id,
              name: freeUpgrade.name,
              actual_total: item.category_details.actual_total,
              gst: item.category_details.gst,
              total: item.category_details.total,
              actual_cabin_fare: freeUpgrade.actual_cabin_fare,
              cabin_fare: freeUpgrade.cabin_fare,
              per_guest: item.category_details.per_guest,
              actual_per_guest: item.category_details.actual_per_guest,
              service_charge: item.category_details.service_charge,
              fuel_surcharge: item.category_details.fuel_surcharge,
              visa_fees: item.category_details.visa_fees,
              decks: freeUpgrade.decks,
            },
            selected: freeUpgrade.code,
            selected_name: freeUpgrade.name,
          };
        }

        // no free upgrade → keep as is
        return item;
      });
      ...
      await createBooking(_payload)
        .unwrap()
        .then((res: any) => {
          window.localStorage.setItem('store', JSON.stringify({ itinerary: selectedItinerary, rooms: updatedData, booking: res, GAData: itemData }));
    ...
    <div className='pb-56 lg:pb-0'>
          ...
          <div className='grid grid-cols-10 mt-4 gap-8'>
            <div className='col-span-10 lg:col-span-6'>
                ...
                <div className='flex mt-2'>
                  {departurePortList?.map((item: any) =>
                    <div
                      ...
                      className={`${item == departurePort ? 'bg-brand-gradient text-white' : 'bg-gray-400 text-gray-100'} w-full px-4 py-3 lg:py-4 flex justify-center cursor-pointer font-semibold text-sm lg:text-base`}
                    >
                      <p>{item}</p>
                    </div>
                  )}
              ...
              <div className="my-4 flex justify-center">
                <Button text='+ Add Cabin' size='sm' type='secondary' disabled={!checkLastRoom(selectedRooms)} handleClick={() => onRoomChange(ADD_ROOM, 0)} />
              </div>
            </div>
            <div className='col-span-4 hidden lg:block'>
              <div className=' sticky top-40 mb-20'>
                ...
                <div className='mt-8'>
                  <Button text={loadingBooking ? 'Loading...' : 'Continue'} disabled={loadingBooking || !checkCabin(selectedRooms)} isLoading={loadingBooking} handleClick={() => proceedToBilling()} className='w-full' />
                </div>
              </div>
            </div>
          </div>
      ...
      <div className='fixed w-full bottom-0 z-30 lg:hidden shadow-[rgba(0,0,0,0.14)_5px_-2px_5px] '>
        <div className='bg-white px-4 py-4 '>
          ...
          <div className='mt-3'>
            <Button text={loadingBooking ? 'Loading...' : 'Continue'} disabled={loadingBooking || !checkCabin(selectedRooms)} isLoading={loadingBooking} handleClick={() => proceedToBilling()} />
          </div>
        </div>
      </div>
