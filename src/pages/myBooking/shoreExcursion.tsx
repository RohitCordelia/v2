...
import { useGetItineraryQuery, useGetShoreExMutation, useUpdateShoreExcursionMutation } from '../../../src/services/itinerary/itinerary';
...
export default function Offers({ }: Props) {
  ...
  const [shoreExc, setShoreEx] = useState([])
  ...
  // const {shoreEx , booking} = location?.state || {}

  let param:any = '/' + booking?.itinerary_id;
  const { data, isSuccess } = useGetItineraryQuery(param)
  useEffect(()=>{
    const _payload:any = { id: booking?.id };
    getShoreEx(_payload)
     .unwrap()
     .then((response: any) => {
      ...
      //    navigate('/manage-booking/shore-ex', { state: { shoreEx: response, booking: booking } })
      //  } else {
      //    navigate('/manage-booking/payment-summery',{state:{resp}});
      //  }
     })
     .catch((response: any) => {
  ...
  useEffect(() => {
    if (store && store?.rooms && data) {
      let res = store?.shore_excursions;
      let newArray = []
      if (res?.guest_updated) {
        newArray = res?.ports
      } else {
        let bookingGuest = booking?.guests;
  ...
  useEffect(() => {
    let val = shoreExc[activePortIndex]
    setSelectedPort(val?.port_name)
    setSelectedGST(val?.gst_pct)
  }, [activePortIndex, shoreExc])
  ...
  const onSelectGuest = (portIndex: any, shoreExIndex: any) => {
    ...
    let shore = shoreExc[portIndex].shore_excursions[shoreExIndex];
    let GAItem = {
      item_id: shore.id,
    ...
    setGuestModal(true);
    setSelectedShoreEx(shoreExc[portIndex].shore_excursions[shoreExIndex]);
    setSelectedShoreExIndex(shoreExIndex);
  }

  let indexsss = shoreExc.findIndex((x: any) => x.port_name === SelectedPort);
  ...
  const proceedToPayment = async () => {
    ...
    const transformedShoreEx = shoreExc?.flatMap((item: any) =>
    ...
    await updateShoreExcursion(_payload)
      ...
      .then((res: any) => {
        ...
        updatedShoreEx.ports = shoreExc

        SaveStore({ ...store, shore_excursions: updatedShoreEx, GADataShoreEx: GAItem });
        navigate('/payment-summary?booking_id=' + booking?.id)
      })
      .catch((res: any) => {
  ...
  const cancelGuestSelection = () => {
    setGuestModal(false)
    setTempShoreEx(JSON.parse(JSON.stringify(shoreExc)))
  }

  return (
    <Layout>
      <div className='bg-gray-100/5'>
        <main className="container mx-auto py-16 lg:pt-28 lg:pb-28 relative grid grid-cols-3 gap-4">
    
          <div className="mx-4 lg:pb-0 lg:py-0 lg:col-span-1 hidden lg:block w-full mb-[18rem] md:mb-72">
            <div className='lg:bottom-40 left-0 lg:left-auto'>
            <div className="flex items-center cursor-pointer mb-6">
              ...
              <div className='grid grid-cols-1 lg:grid-cols-1 lg:border-none lg:mb-3.5 bg-white z-10 lg:z-0 w-full shadow-[rgba(0,_0,_0,_0.15)_0px_0px_7px_2px] rounded-lg'>
               <p className='text-center text-brand-secondary bg-brand-secondary/[0.1] py-2 font-bold rounded-t-md ' >Booking ID: C-2212060002</p>
                ...
                <div className='grid grid-cols-10 lg:pt-6 pb-0 pt-5'>
                  <div className='px-4 col-span-10 lg:col-span-10'>
                    <div className='flex items-center'>
                      ...
                      />
                      <p className='text-sm col-span-8 font-bold leading-5 lg:text-lg'>{Itinerary?.ports?.map((port: any, index: number) => {
                        return (
                          <span key={index}>
                            <span>{port['name']}</span>
                  ...
                  <div className='px-4 col-span-10 lg:col-span-10 mt-3'>
                    ...
                    {shoreExAmount?.port_code?.length ?
                      <div>
                        ...
                        {shoreExAmount?.port_code?.map((x:any, i:number) =>
                    ...
                    <div className='flex items-center justify-between px-4 py-4'>
                      ...
                      <div>
                        <p className="text-lg font-bold text-brand-primary">
                          {`₹ ${Math.round(store.booking.sub_total * (booking?.payment_option_rule ? 0.25 : 1)) + Math.round(shoreExAmount.adult) + Math.round(shoreExAmount.children)}`}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              {window.innerWidth > 640 && <Button text='Proceed' size='sm' handleClick={() => proceedToPayment()} className="hidden lg:block w-full mr-7" />}
            </div>
          </div>
          ...
          <div className="mx-4 mb-4 lg:col-span-2 col-span-3 lg:mt-0 ">
            <div className="mt-4 lg:mt-0 flex justify-end items-center">
              ...
              <div className="flex items-center cursor-pointer lg:mb-6">
                    <img
                        src="https://images.cordeliacruises.com/cordelia_v2/public/assets/back-arrow-icon.svg"
                        alt="arrow"
                        onClick={() => navigate(-1)}
                ...
                <p className="text-sm mr-2 font-semibold underline text-brand-primary lg:text-base" onClick={() => skip()}>
                  Skip
                </p>
              </div>
            </div>
            <div className='mt-6 lg:mt-8'>
              <div className=''>
                <ShoreExcursionHeader ports={shoreExc} callback={(e) => setActivePortIndex(e)} />
              </div>
              <div className='bg-white top-shadow1'>
                <div className='grid grid-cols-1 lg:px-8 pt-6 lg:pt-8'>
                  {shoreExc[indexsss]?.shore_excursions ? shoreExc[indexsss]?.shore_excursions.map((v: any, i: number) => {
                            ...
                            <div className='px-4 py-2 lg:py-0'>
                              ...
                              <div className='flex justify-between items-center mt-3'>
                                <div>
                                  <Button
                                    text={
                                      v.adult_count || v.children_count
                                        ? 'View Guest'
                                        : 'Select Guests'
                                    }
                                    size="sm"
      ...
      <div className='fixed w-full bottom-0 z-30 lg:hidden'>
        <div className='grid grid-cols-5 bg-white px-4 py-4 shadow-[rgba(0,0,0,0.14)_5px_-2px_5px] border-t border-gray-100/20'>
          <div className='col-span-3 self-center'>
            <p className='text-brand-primary text-base font-bold'>Total Fare: ₹ {FormatAmount(Math.round(store.booking?.sub_total * (booking?.payment_option_rule ? 0.25 : 1)) + shoreExAmount.adult + shoreExAmount.children)}</p>
            <div className='flex items-center mt-1'
              onClick={() => {
                setBreakupModal(true)
              }}
            >
              <p className='text-xs font-semibold text-brand-blue'>View Price Breakup</p>
              <img
      ...
      <Modal show={breakupModal} align={'center'} className="drop-shadow modal-open bg-white w-full center bottom-0 rounded-lg lg:rounded-2xl border right-0 m-auto absolute" onClose={() => setBreakupModal(false)}>
        <div className='relative h-full'>
          <div onClick={() => setBreakupModal(false)} className='flex items-center justify-center shadow-allSide absolute -top-4 left-0 right-0 mx-auto bg-white w-10 h-10 rounded-full '>
              ...
              {shoreExAmount?.port_code?.length ?
                <div>
                  <div className='flex items-center justify-between pt-4'>
                    ...
                    <p className="text-lg font-bold text-brand-primary">
                      {`₹ ${FormatAmount(Math.round(store.booking.sub_total* (booking?.payment_option_rule ? 0.25 : 1 )) + shoreExAmount.adult + shoreExAmount.children)}`}
                    </p>
                  </div>
                </div>
                : null}
