...
export default function CabinSelection({ }: Props) {
  // const {id}=useParams();
  ...
  setLoading(true)
    const _payload: any = {
      category_id:'b26e91aa-e4aa-4047-b54b-388458767abc',
      beds: 1,
      id: '10a75f19-1812-4db5-863c-6a3f994219d1',
      deckno: 7
    };
  ...
  useEffect(() => {
    const deck = {"description":"INTERNET CAFÉ","price":0,"id":7,"selectable":true,"images":[{"src":"/static/booking/interior/1.jpg","width":"1280","height":"720"},{"src":"/static/booking/interior/2.jpg","width":"1280","height":"720"}],"obstructed":false}
    getRoomSelectionLayout(deck);
    window.scrollTo(0, 0)
  }, [])
  ...
  const RoomLayout = ({ roomDeck }: any) => {
    return roomDeck.map((room: any, key: number) => {
      ...
      return (
        <div key={key}
          className={`
          border rounded-[4px] flex px-3 py-2 justify-center items-center text-center text-xxs lg:text-xs font-semibold uppercase select-none 
          ${room.available && "cursor-pointer"} 
          ${room.number === selectedRoom?.number || roomExist ? 'bg-success border-success text-white' : room.available ? 'bg-white border-gray-100' : 'bg-gray-100/20 border-gray-100/20'}
          `}
  ...
  const isComplete = !currentRoom?.filter((room: any) => !room.room_number).length
      ...
      <section className='px-40 mt-20' >
        ...
        <div className='flex gap-3 items-start lg:py-4 px-2 py-2 mt-4 rounded-md lg:px-5 bg-brand-secondary/[0.1]' >
          ...
          <div className='flex flex-col gap-1' >
            <p className=' font '>Please select a room from below available rooms</p>
          </div>
        </div>
        <div className="grid  grid-cols-12 gap-4">
          <div className="col-span-12 lg:col-span-7 lg:order-2 ">

            <div style={{ gridArea: 'exp' }} className="flex justify-center gap-8  text-xs mt-6 mb-8">
              <div className='flex items-center'>
                <div className='rounded-sm shadow-allSide border h-5 w-5 mr-2' />
                <p className='text-sm lg:text-base font-semibold'> Available</p>
              </div>
              <div className='flex items-center'>
                <div className='rounded-sm border border-gray-100/20 bg-gray-100/20 border-bg-gray-100/20 h-5 w-5 mr-2' />
                <p className='text-sm lg:text-base font-semibold'> Occupied</p>
              </div>
              <div className='flex items-center'>
                <div className='rounded-sm  bg-success h-5 w-5 mr-2' />
                <p className='text-sm lg:text-base font-semibold'> Selected</p>
              </div>
            </div>
            <div className='overflow-scroll h-[80%] px-4 text-center bg-white custom-scrollbar'>
        ...
        <div className='h-[12%] px-4 mt-2 text-center rounded-b-lg lg:rounded-b bg-white'>
          ...
          <div className="col-span-12 lg:col-span-5 lg:order-2 mt-16 ">
          <div className='shadow-allSide px-4 py-4 fixed lg:sticky rounded-md  ' >
          <img className='' src='https://images.cordeliacruises.com/static/booking/interior/2.jpg' />
          <div className="grid px-3 py-4 border-b border-gray-300 grid-cols-10">
          <div className="col-span-1 lg:col-span-3 ">
            <p className='text-sm text-gray-200' >Cabin Type</p>
            <p className='text-md font-bold text-brand-primary' >Ocean View</p>
            </div>
            <div className="col-span-1 lg:col-span-4  border-l border-gray-300 ">
            <p className='text-sm text-gray-200 ml-4 ' >Selected Cabin</p>
            <p className='text-md font-bold text-brand-primary ml-4' >3071</p>
            </div>
          <div className="col-span-1 lg:col-span-3">
            <a className='cursor-pointer hidden lg:inline-block rounded-[4px] bg-brand-primary text-white py-3 font-semibold px-8 text-md' >Proceed</a>
            </div>
            </div>
          </div>
          </div>
...
const ShipFront = (deckk: any) => {
  let deck = [{
    id: 'lfmkjreiowur04iur0jjfndw'
  }]
  return (
    <div style={{ gridArea: 'dck' }} className="relative">
      ...
      <div className="absolute text-center w-full lg:text-sm text-xs font-semibold top-12 lg:top-28 h-full">Deck {deck ? deck?.id : null}</div>
    </div>
  )
}
