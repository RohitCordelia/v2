...
import { FormatToString, ordinal_suffix_of } from '../../../src/utils/formatter/formatter';
import { Input } from '../../../src/components/UI/Forms/Inputs';
import { useForm } from 'react-hook-form';
import { FullName, Phone, Email, Pincode, GSTIN, Address, PanNo } from '../../../src/utils/validations/formValidations';
import Modal from '../../components/UI/ModalCenter';
import { useGetAddressFromPincodeQuery, useInitPaymentNewMutation, useApplyCouponMutation, useRemoveCouponMutation, useVerifyPanMutation, useVerifyGSTMutation } from '../../../src/services/payments/payment';
...
export default function QuotationPaymentSummary({ }: Props) {
  ...
  useEffect(() => {
    if (quotationData && quotationData.booking) {
    ...
    if (quotationData?.booking?.is_international) {
  ...
  }, [quotationData, travelExpense, nri])
  ...
  const initPayment = () => {
    let billing = {}
    if (!nri && quotationData?.booking?.is_international) {
    ...
    completePayment(_payload)
      ...
      .then((res: any) => {
        setLoading(false);
        navigate('/quotation/payment-method', { state: { paymentData: res, billingAddress: billingAddress, quotationData: quotationData, QUOTATION_ID: QUOTATION_ID, payment_for: PaymentType, promo_code: couponCode } });
      })
      .catch((res: any) => {
        console.log('Error: ', res);
        setLoading(false);
  ...
  const applyCouponCode = async (coupon: any, booking_id: any) => {
    if (couponArray.find((v) => v === coupon)) {
      const _payload = {
        booking_id: booking_id,
        promo_code: coupon,
      };
      setCouponLoading(true)
      await applyCoupon(_payload)
        .unwrap()
        .then((res: any) => {
          setBookingLoading(false)
          ...
          dispatch(setCouponCodeProp(pay));
          setQuotationData(res)
        ...
        .catch((res: any) => {
          ...
          setCouponAppliedMessage('Invalid Coupon Code');
          console.log('Error: ', res);
        });
  ...
  const removeCouponCode = () => {
    ...
    removeCoupon(_payload)
      .unwrap()
      .then((res: any) => {
        let coup = ''
        dispatch(setCouponCodeProp(coup));
        // SaveStore({ ...store, coupon_code: null });
        setQuotationData(res)
        setCouponApplied(false);
  ...
  console.log('roh aaa', isPanVerified);
      ...
      <main className="container mx-auto py-24 lg:pt-36 px-3">
        <div>
          ...
          <div className="border border-gray-400 shadow-allSide rounded-lg mb-4">
            <div className="grid grid-cols-2 border-b border-gray-300 px-6 py-3">
              <div className="">
                ...
                <p className="text-base lg:text-xl text-brand-primary font-bold mt-0.5">{`${quotationData?.booking?.number || ''}`}</p>
              </div>
              <div className="text-right">
                ...
                <div className='flex w-full justify-end items-center'>
                  {quotationData?.itinerary?.discount_pct != 0 ? <p className="text-sm lg:text-base text-gray-100 line-through mr-3"> {` ${' '} ₹ ${quotationData?.booking?.actual_total + (tcsObject?.amount || 0)}`} </p> : null}
                  <p className="text-base lg:text-2xl text-brand-primary font-bold">{`₹ ${Math.floor(
                    quotationData?.booking?.current_payable || 0
                  )}`}</p>
                </div>
              </div>
            </div>

            <div className="grid grid-cols-2 mt-3 gap-4 lg:gap-10 lg:p-10 p-4">
              <div className="">
                      ...
                      <div key={i} className="mb-2 lg:mb-3">
                        {i === 0 ? (
                          <div>
                            ...
                            <p className="text-xs lg:text-base font-medium">{`${i === 0 ? val.departure : val.arrival}`}</p>
                          </div>
                        ) : i !== 0 &&
                          quotationData?.itinerary?.ports.length - 1 === i ? (
                          <div className='pt-2'>
                            <div className="flex items-center mb-1 lg:mb-2">
                              <img
                                src="https://images.cordeliacruises.com/cordelia_v2/public/assets/location-icon.svg"
          ...
          <div className="border border-gray-400 shadow-allSide rounded-lg px-4 lg:px-6 py-4 lg:py-6 mb-4">
            ...
            {quotationData?.booking?.is_international &&
          ...
          {quotationData?.booking?.is_international && !nri &&
            <>
              <div className="border border-gray-400 shadow-allSide rounded-lg mb-4">
            ...
            <div>
              ...
              >
                <div className='border-t mt-5 pt-5 border-gray-300'>
                  {quotationData?.booking?.rooms.map((room: any, index: number) => {
                    return (
                      <div className="grid grid-cols-1 mb-2" key={index}>
                        <div className='flex w-full justify-between mb-0.5 items-center'>
                          <p className="text-sm lg:text-lg font-semibold">{`CABIN ${index + 1} FARE`}</p>
                          <div className='text-right'>
                            {/* <p className="text-sm lg:text-lg font-semibold line-through">{`₹ ${Math.floor(room.actual_cabin_fare)}`}</p> */}
                            <p className="text-sm lg:text-lg font-semibold">{`₹ ${Math.floor(room.actual_cabin_fare)}`}</p>
                          </div>
                        </div>
                        <div className='flex w-full justify-between mb-0.5 items-center'>
                          <p className="text-sm lg:text-lg font-semibold">Service Charge & Levis</p>
                          <p className="text-sm lg:text-lg font-semibold">{`₹ ${room.service_charges}`}</p>
                        </div>
                        <div className='flex w-full justify-between mb-0.5 items-center'>
                          <p className="text-sm lg:text-lg font-semibold">Fuel Srucharge</p>
                          <p className="text-sm lg:text-lg font-semibold">{`₹ ${room.fuel_surcharge}`}</p>
                        </div>
                        {/* <div className="col-span-2">
                          <p className="text-sm lg:text-lg font-semibold">{`CABIN ${index + 1} FARE`}</p>
                          <p className="text-sm lg:text-lg font-semibold mt-1.5">Service Charge & Levis</p>
                          <p className="text-sm lg:text-lg font-semibold mt-1.5">Fuel Srucharge</p>
                        </div>
                        <div className="text-right">
                          <p className="text-sm lg:text-lg font-semibold">{`₹ ${room.cabin_fare}`}</p>
                          <p className="text-sm lg:text-lg font-semibold mt-1.5">{`₹ ${room.service_charges}`}</p>
                          <p className="text-sm lg:text-lg font-semibold mt-1.5">{`₹ ${room.fuel_surcharge}`}</p>
                        </div> */}
                      </div>
                    );
                  })}
                  ...
                  {quotationData?.booking?.protection_plan && quotationData?.booking?.protection_plan?.amount > 0 &&
                    <div className="grid grid-cols-3 mt-3">
                      ...
                      <div className="text-right">
                        <p className="text-sm lg:text-lg font-semibold">₹{quotationData?.booking?.protection_plan?.amount}</p>
                      </div>
                    </div>
                  }

                  {quotationData?.booking?.cabin_fare_discount ?
                    <div className="grid grid-cols-1 mt-3">
                      <div className="flex w-full justify-between items-center">
                        ...
                        <div className="text-right">
                          <p className="text-sm lg:text-lg font-semibold text-brand-green">{`- ₹ ${parseInt(quotationData?.booking?.cabin_fare_discount).toFixed()}`}</p>
                        </div>
                      </div>
                    </div>
                    : null
                  ...
                  <div className="grid grid-cols-1 mt-3">
                    <div className="flex w-full justify-between items-center">
                      ...
                      <div className="text-right">
                        {/* <p className="text-sm lg:text-lg font-semibold line-through">{`₹ ${parseInt(quotationData?.booking?.actual_sub_total).toFixed()}`}</p> */}
                        <p className="text-sm lg:text-lg font-semibold">{`₹ ${parseInt(quotationData?.booking?.sub_total).toFixed()}`}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 mt-3">
                    <div className="col-span-2">
                      ...
                      <p className="text-xs lg:text-base text-gray-100 font-semibold mt-1">
                        GST:
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-sm lg:text-lg font-semibold">{`₹ ${(quotationData?.booking?.gst)}`}</p>
                      <p className="text-xs lg:text-base text-gray-100 font-semibold mt-1.5">{`₹ ${quotationData?.booking?.gst}`}</p>
                    </div>
                  </div>
                  {tcsObject &&
                    <>
                      <div className="grid grid-cols-1 mt-3">
                        <div className="flex justify-between">
                          ...
                          <div className="text-right">
                            {quotationData?.itinerary?.discount_pct != 0 ? <p className="text-sm lg:text-lg line-through font-bold">{`₹ ${FormatToString(
                              quotationData?.booking?.actual_total
                            )}`}</p> : null}
                  ...
                  <div className="grid grid-cols-1 mt-3">
                    <div className="flex justify-between items-center">
                      <p className="text-lg lg:text-2xl font-bold text-brand-primary">Grand Total:</p>
                      <div className="text-right">
                        {quotationData?.itinerary?.discount_pct != 0 ?<p className="text-sm lg:text-base line-through text-gray-100">{`₹ ${FormatToString(
                          quotationData?.booking?.actual_total + (tcsObject?.amount || 0)
                        )}`}</p> : null}
                        <p className="text-lg lg:text-2xl font-bold text-brand-primary">{`₹ ${FormatToString(
        ...
        <div>
          <div className={`border p-4 lg:p-6 mt-4 rounded border-gray-400 shadow-allSide`}>
            ...
            <div className="border-t mt-6 border-gray-300">
              ...
              {quotationData && quotationData?.booking && quotationData?.booking.is_international ? (
                <div>
                  <p className="italic text-xxs lg:text-xs font-medium lg:font-semibold mt-3 mb-2">
                    Agree to pay TCS charges applicable in a financial year as per below:
                  </p>
                  <p className="italic text-xxs lg:text-xs font-medium lg:font-semibold mt-0 mb-0.5">
                    If the Tour expenses are less than 7 lacs then 5% TCS will be levied
                  </p>
                  <p className="italic text-xxs lg:text-xs font-medium lg:font-semibold mt-0 mb-0.5">
                    If the Tour expenses are more than 7 lacs then 20% TCS will be levied
                  </p>
                  <p className="italic text-xxs lg:text-xs font-medium lg:font-semibold mt-0 mb-0">
                    TCS will apply to the total cost (i.e.(Cabin fare + Service Charge & Levies + Fuel charge) *GST)*TS)
                  </p>
                </div>
              ) : null}
            </div>
