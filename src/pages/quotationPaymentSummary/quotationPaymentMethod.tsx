...
export default function PaymentMethod({ }: Props) {
  ...
  console.log('ItineraryProp', ItineraryProp);
  
  const { paymentData, billingAddress, quotationData, QUOTATION_ID, payment_for, promo_code } = state;
  ...
  useEffect(() => {
    if (quotationData && quotationData.booking.split_payment_options.available && payment_for === 'split_payment') {
      let obj = quotationData.booking.split_payment_options.options.find(o => o.paid === false);
      setTotalAmount(obj.amount)
    } else {
      if (quotationData.booking.payment_option_rule && quotationData.booking.payment_option_rule === '25%') {
        setTotalAmount(quotationData.booking.current_payable)
      } else {
        setTotalAmount(quotationData.booking.total)
      }
    }
  }, [quotationData])
        ...
        {paymentData ? (
          <div className="">
            <div className="pb-3 flex items-center cursor-pointer"
              onClick={() => navigate('/quotation/payment-summary?id=' + QUOTATION_ID)}
            >
              <img
            ...
            {/* <div className="border-x-2 border-t-2 border-gray-300 rounded-t p-3">
              <div
                className="flex items-center cursor-pointer"
                onClick={() => setPaymentRadio(WALLET)}
              >
                {paymentRadio === WALLET ? (
                  <img
                    src="https://images.cordeliacruises.com/cordelia_v2/public/assets/button-select.svg"
              ...
              {paymentRadio === WALLET && (
                <div>
                  <div className="mt-4 grid grid-cols-2 gap-2 gap-y-3">
                    {WalletQuickList?.map((wallet: any, index: number) => {
                      return (
                        <div
                          className="mx-2 cursor-pointer flex flex-col items-center justify-around"
                          key={index}
                          onClick={() =>
                            setWalletValue('wallet', wallet.wallet_name)
                          }
                        >
                          <img
                            src={`https://images.cordeliacruises.com/cordelia_v2/public/assets/${wallet.wallet_name}.svg`}
                            // style={{
                            //     width: 57,
                            //     height: 57,
                            // }}
                            ...
                            className="self-center mx-auto max-h-5 mb-2"
                          />
                          <p className="text-sm">{wallet.wallet_name}</p>
                        </div>
                      );
                    })}
                  </div>
                  <Select
                    name="wallet"
                    options={WalletList.map((wallet: any) => {
                      return {
                        code: wallet.wallet_name,
                    ...
                    error={errorsWallet && errorsWallet?.wallet}
                    errorText={errorsWallet && errorsWallet.wallet?.message}
                  />
                  <button
                    onClick={handleSubmit(onSubmitWallet)}
                    className="bg-brand-primary w-full text-white text-sm py-4 font-bold mt-3 disabled:bg-brand-primary/60 disabled:cursor-not-allowed"
                    disabled={loading}
                  >
                    {loading ? (
