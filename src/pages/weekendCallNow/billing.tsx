...
import { FullName, Phone, Email, GSTIN, Required } from '../../../src/utils/validations/formValidations';
import { useVerifyPanMutation, useVerifyGSTMutation } from '../../../src/services/payments/payment';
import { GetStore, SaveStore } from '../../../src/utils/store/store';
import Accordian from '../../components/UI/Accordion/accordion_basic';
import { useInitPaymentNewMutation } from '../../../src/services/payments/payment';
import { useGetBookingDetailMutation } from '../../../src/services/itinerary/itinerary';
import Modal from '../../components/UI/Modal';
import { FormatCardNumber, FormatToString, UnFormatCardNumber, ordinal_suffix_of } from '../../../src/utils/formatter/formatter';
import { FormatAmount } from '../../../src/utils/formatter/formatter';
import Header from "./component/header";
import { TiggerGAInitiatePayment, TiggerFBCompleteRegistrationEvent } from '../../../src/components/Analytics/events';
...
export default function WeekendBilling({ }: Props) {
    ...
    const initPayment = () => {
        ...
        completePayment(_payload)
            ...
            .then((res: any) => {
                setLoading(false);
                navigate('/payment-method', { state: { paymentData: res, billingAddress: billingAddress, bookingData: bookingData, payment_for: PaymentType } });
            })
            .catch((res: any) => {
                console.log('Error: ', res);
                setLoading(false);
    ...
    return (
        <>
                ...
                <div>
                    ...
                    <div className='grid grid-cols-12 mt-4'>
                        <div className='col-span-12 lg:col-span-5 lg:pl-10 lg:order-2'>
                            <div className='px-4 py-4 lg:py-6 border border-[#185DA0]/20 rounded-t shadow-sm bg-[#F4F8FF]'>
                                <div className='flex justify-between items-start'>
                                    <p className='text-base lg:text-xl  font-semibold'>{itinerary.portName}</p>
                                    <div
                                        className='flex items-center bg-[#185DA0] px-3 py-1.5 lg:px-4 lg:py-1.5 rounded cursor-pointer'
                                        onClick={() => navigate(-1)}
                                    >
                                        <p className='text-white text-sm lg:text-base mr-1 lg:mr-2'>Edit</p>
                                        <img className='lg:w-3' src="https://images.cordeliacruises.com/cordelia_v2/public/assets/edit-icon.svg" alt="" />
                                    </div>
                                </div>
                                <div className='flex items-start justify-between mt-4 lg:mt-8'>
                                    <div className='text-center'>
                                        <p className='text-xs lg:text-sm  font-semibold'>{moment(itinerary.start_date, 'DD/MM/YYYY').format('MMM Do, YYYY')}</p>
                                        <p className='text-xxs text-gray-100 font-semibold'>
                                            {moment(itinerary?.ports[0].departure, 'ddd, DD MMM HH:ss A').format('dddd hh:ss A')}
                                        </p>
                                    </div>
                                    ...
                                    <div className='text-center'>
                                        <p className='text-xs lg:text-sm  font-semibold'>{moment(itinerary.end_date, 'DD/MM/YYYY').format('MMM Do, YYYY')}</p>
                                        <p className='text-xxs text-gray-100 font-semibold'>
                                            {moment(itinerary?.ports[itinerary?.ports.length - 1].arrival, 'ddd, DD MMM HH:ss A').format('dddd hh:ss A')}
                                        </p>
                                    </div>
                                </div>
                                    ...
                                    {amenities?.map((item: any) => {
                                        return (
                                            <div className='px-2 lg:px-3 py-1.5 mr-1 flex items-center'>
                                                <img className='h-5 lg:h-4 mr-1 lg:mr-2' src={item.icon} alt="" />
                                                <p className='text-xxs text-gray-600 font-medium'>{item.title}</p>
                                            </div>
                                        )
                                    })}
                                </div>
                                ...
                                return (
                                    <div className='px-4 py-4 lg:py-6 border border-[#185DA0]/20 shadow-sm bg-[#F4F8FF] flex justify-between'>
                                        <div>
                                            ...
                                            <p className='text-sm lg:text-base font-semibold mt-1'>{room.selected_name}</p>
                                        </div>
                                        <div>
                                            <p className='text-xs lg:text-sm font-medium'>Guest No</p>
                                            <p className='text-sm lg:text-base font-bold text-brand-primary mt-1'>{room.adults + room.children + room.infants}</p>
                                        </div>
                                    </div>
                                )
                            })}
                            ...
                            <div className='mt-4'>
                                <div className=' py-4 lg:py-6 border border-gray-400 bg-white rounded shadow-allSide'>
                                    <div className='px-4 flex items-start justify-between border-b border-gray-300 pb-4 lg:pb-6'>
                                        <p className='text-base lg:text-xl font-bold'>Price Details</p>
                                        <p onClick={() => setHideBreakup(!hideBreakup)} className='text-sm lg:text-base text-[#008CFF] font-medium cursor-pointer'>{hideBreakup ? 'View Price Breakup' : 'Hide Price Breakup'}</p>
                                    </div>
                                    ...
                                    {!hideBreakup &&
                                        <div className='pt-6'>
                                                ...
                                                return (
                                                    <div className='mb-3 px-4'>
                                                        <div className='flex justify-between mb-1'>
                                                            <p className="text-sm lg:text-base  mb-0.5 font-medium">{room.category_name} Cabin</p>
                                                            <p className="text-sm lg:text-base  mb-0.5 font-medium">₹ {FormatAmount(room.actual_cabin_fare)}</p>
                                                        </div>
                                                        <div className='flex justify-between mb-1'>
                                                            <p className="text-sm lg:text-base  mb-0.5 font-medium">Service Charge & Levies</p>
                                                            <p className="text-sm lg:text-base  mb-0.5 font-medium">₹ {FormatAmount(room.service_charges)}</p>
                                                        </div>
                                                        <div className='flex justify-between mb-1'>
                                                            <p className="text-sm lg:text-base  mb-0.5 font-medium">Fuel Surcharge</p>
                                                            <p className="text-sm lg:text-base  mb-0.5 font-medium">₹ {FormatAmount(room.fuel_surcharge)}</p>
                                                        </div>
                                                        {/* <div className='flex justify-between'>
                                            ...
                                            <div className='px-4 pb-3'>
                                                <div className='flex justify-between'>
                                                    <p className="text-sm lg:text-base  mb-0.5 font-semibold">Sub-total</p>
                                                    <p className="text-sm lg:text-base  mb-0.5 font-semibold">₹ {FormatAmount(booking.sub_total)}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div className='bg-[#F4F8FF] px-4 py-3'>
                                                <div className='flex justify-between mb-1'>
                                                    ...
                                                    <p className="text-sm lg:text-base  mb-0.5 font-medium">₹ {FormatAmount(booking?.gst)}</p>
                                                </div>
                                                <div className='flex justify-between'>
                                                    <p className="text-sm lg:text-base  mb-0.5 font-medium text-gray-200">GST</p>
                                                    <p className="text-sm lg:text-base  mb-0.5 font-medium text-gray-200">₹ {FormatAmount(booking?.gst)}</p>
                                                </div>
                                            </div>
                                        </div>
                                    }
                                    ...
                                    <div className='px-4 pt-2 lg:pt-4'>
                                        <div className='flex justify-between items-center mt-2'>
                                            <p className='text-base lg:text-2xl font-bold'>Total Fare</p>
                                            <div className='text-right'>
                                                {itinerary?.discount_pct != 0 ?
                                                    <p className='text-sm lg:text-base font-semibold text-gray-100 line-through'>₹ {FormatAmount(booking?.actual_total)}</p>
                                                    : null}
                                                <p className='text-base lg:text-2xl font-bold text-brand-primary'>₹ {FormatAmount(booking?.total)}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className='mt-4 hidden lg:block'>
                                    {concent ?
                                        <button
                                            disabled={loading || (gst ? !isGstVerified : null) || !isPanVerified}
                        ...
                        <div className='col-span-12 lg:col-span-7 mt-4 lg:mt-0'>
                            ...
                            <div className={`border-2 p-4 mt-2 rounded border-gray-400 shadow-allSide`}>
                                <div className="flex items-center">
                                    <input
                                        type="checkbox"
                                        ...
                                        checked={gst}
                                    />
                                    <p className={`text-sm lg:text-lg font-semibold `}>I Have a GST Number</p>
                                </div>
                                <p className={` text-xxs lg:text-sm mt-2 text-gray-100`}>Kindly submit GST details for seamless filing of your tax return</p>
                                {gst && (
                            ...
                            <div className={`border-2 p-4 mt-4 rounded border-gray-400 shadow-allSide`}>
                                <div className="flex items-center">
                                    <input
                                        type="checkbox"
                                        className="mr-2 cursor-pointer"
                                    ...
                                    />
                                    <p className={`text-sm lg:text-lg font-semibold `}>By clicking Agree and Continue, I hereby:</p>
                                </div>
                                <p className={` text-xxs lg:text-sm mt-2 text-gray-100 lg:leading-6`}>Agree and consent to the Privacy Policy, Cancellation and Rescheduling Policy, Passenger Cruise Ticket Contract and all other Terms and Conditions as appear hereinabove and on website www.cordeliacruises.com.</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className='fixed w-full bottom-0 z-30 lg:hidden shadow-[rgba(0,0,0,0.14)_5px_-2px_5px]'>
                <div className='bg-white px-4 py-4'>
                    <div className='flex justify-between items-center mb-1'>
                        <p className='text-xs font-semibold'>{itinerary?.nights}N/{itinerary?.nights + 1}D</p>
                    ...
                    <div className='flex justify-between items-center mb-1'>
                        ...
                        <div className='text-right'>
                            {itinerary?.discount_pct != 0 ?
                                <p className='text-xs lg:text-base font-semibold text-gray-100 line-through'>₹ {FormatAmount(booking?.actual_total)}</p>
                                : null}
                            <p className='text-base font-bold text-brand-primary'>₹ {FormatAmount(booking?.total)}</p>
                        </div>
                    </div>
                    <div className='flex justify-between items-center'>
                        <div className='flex items-center'>
                            ...
                            <img
                                ...
                                alt="Cruise"
                            />
                            <p className='text-xxs font-semibold'>{rooms.reduce((sum: any, item: any) => sum + item.adults + item.children + item.infants, 0)} Guest</p>
                        </div>
                ...
                <div className='pb-5 lg:pb-10'>
                    ...
                    <div className='flex items-start border-t p-5' onClick={() => setPaymentType('single_payment')}>
                        ...
                        <div className='ml-2'>
                            ...
                            <p className='text-xxs font-semibold'>I can pay {bookingData?.booking?.total} in one transaction</p>
                        </div>
                    </div>
            ...
            <Modal show={SplitModal} align={'center'} className=" bg-white w-[90%] lg:w-2/4 center bottom-1/4 rounded-lg lg:rounded border overflow-hidden left-0 right-0 m-auto	" onClose={() => setSplitModal(false)}>
