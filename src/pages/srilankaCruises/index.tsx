...
export default function Home(props: Props) {
  ...
  const offers = [
    ...
    // },
    // {
    //   title: 'Inclusive of VISA',
    //   description: [
    //     'No additional payment for VISA',
    //     'Applicable only on 5-night Sri Lanka sailings',
    //     'Valid on bookings made for single, double, and triple occupancies only',
    //     'Available for limited inventory per sailing',
    ...
    //   description: [
    //     'Select spirit and mixers included at no extra charges for all the guests',
      ...
      <main>
        <section className="">
          <Banner data={banner ? banner : ''} />
           <LeadGenForm page_code='srilanka' />
              ...
              {offers.map((val, index) => (
                <div className="flex justify-between shadow-allSide" key={index}>
                  <div className="border border-gray-300 w-full flex justify-between items-center px-4 py-3.5 rounded-l">
                    ...
                    {/* <p className='lg:text-sm text-xxs ml-4 text-gray-1100 font-semibold underline whitespace-nowrap cursor-pointer' onClick={() => setDescription(val.description)}>View More</p> */}
                  {/* </div> */}
